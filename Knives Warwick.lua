...
AddLoadCallback(function()
    ...
    Q = _Spell({Slot = _Q, DamageName = "Q", Range = 350, Delay = 0.25, Type = SPELL_TYPE.SELF}):AddDraw()
    W = _Spell({Slot = _W, DamageName = "W", Range = 0, Delay = 0.25, Type = SPELL_TYPE.SELF}):AddDraw()
    E = _Spell({Slot = _E, DamageName = "E", Range = 375, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    Ignite = _Spell({Slot = FindSummonerSlot("summonerdot"), DamageName = "IGNITE", Range = 600, Type = SPELL_TYPE.TARGETTED})
    R = _Spell({Slot = _R, DamageName = "R", Range = 900, Width = 150, Delay = 0.25, Speed = 1200, Collision = true, Aoe = false, Type = SPELL_TYPE.Linear}):AddDraw()
    ...
    Menu:addSubMenu(myHero.charName.." - Keys Settings", "Keys")
        ...
        Menu.Keys:addParam("Flee", "Marathon Mode", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("Z"))
...
function Combo()
    ...
    if ValidTarget(target) then
		...
		if Warwick.Combo.useR and isReady(_R) and distance <= rRange ^ 2 then
		...
		if target and ValidTarget(target) then
			local distance = GetDistanceSqr(target)
			if Warwick.Combo.useQ == 2 and isReady(_Q) and distance <= 350 ^ 2 then
    			CastSpell(_Q, target)
    			CastSpell2(_Q, D3DXVECTOR3(myHero.x, myHero.y, myHero.z))
    		elseif Warwick.Combo.useQ == 3 and isReady(_Q) and distance <= 350 ^ 2 then
    			CastSpell(_Q, target)
    		end
    		if Warwick.Combo.useE == 2 and not eActive and isReady(_E) and distance <= 375 ^ 2 then
    			CastSpell(_E)
    		elseif Warwick.Combo.useE == 2 and eActive and isReady(_E) and distance <= 375 ^ 2 then
    			CastSpell(_E)
    		elseif Warwick.Combo.useE == 3 and not eActive and isReady(_E) and distance <= 375 ^ 2 then
    			CastSpell(_E)
    		end
		end
